...
public class ApplicantSearchController {
    ...
    public ResponseEntity<Map<String, Object>> searchApplicants(
        ...
        try {
            ...
            if (searchDTO.getSize() == null || searchDTO.getSize() <= 0 || searchDTO.getSize() > 50) {
            ...
            response.put("totalPages", applicantsPage.getTotalPages());
            response.put("totalResults", applicantsPage.getTotalElements());
            response.put("hasNext", applicantsPage.hasNext());
            response.put("hasPrevious", applicantsPage.hasPrevious());
            response.put("pageSize", applicantsPage.getSize());
            ...
            System.err.println("[" + java.time.LocalDateTime.now() + "] Lỗi tìm kiếm ứng viên: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("message", "Lỗi server trong quá trình tìm kiếm. Vui lòng thử lại sau."));
        }
    }
    ...
    public ResponseEntity<ApplicantDetailDTO> getApplicantDetails(@PathVariable Integer id) {
        try {
            ...
            System.err.println("[" + java.time.LocalDateTime.now() + "] Lỗi lấy chi tiết ứng viên ID=" + id + ": " + e.getMessage());
